...
import { getSearchedByIdURL } from "../lib/getSearchedByIdURL";
...
export const REQUEST_URLS = {
  ...
  trendings: {
    all: (period: TPeriod, page: number) =>
      `/3/trending/all/${period}?language=en-US&page=${page}`,
    movie: (period: TPeriod, page: number) =>
      `/3/trending/movie/${period}?language=en-US&page=${page}`,
    tv: (period: TPeriod, page: number) =>
      `/3/trending/tv/${period}?language=en-US&page=${page}`,
  },
  searchBy: {
    genres: (arrayOfIds: number[], mainType: TGenresFor, page: number) =>
      getSearchedByIdURL(arrayOfIds, mainType, page),
    keyWord: (query: string, page: number, mainType: TGenresFor) =>
      `/3/search/${
        mainType === "tvSeries" ? "tv" : "movie"
      }?query=${query}&include_adult=false&language=en-US&page=${page}`,
    id: (id: number, mainType: TGenresFor) =>
      `/3/${mainType === "tvSeries" ? "tv" : "movie"}/${id}?language=en-US`,
  ...
  images: {
    ...
    getImages: (id: number, mainType: TGenresFor) =>
      `/3/${mainType === "tvSeries" ? "tv" : "movie"}/${id}/images`,
  },
};
